...
# Mock MT5 Classes for Linux compatibility
class MockMT5Constants:
    """Mock MT5 constants for Linux environments"""
    ORDER_TYPE_BUY = 0
    ORDER_TYPE_SELL = 1
    TRADE_ACTION_DEAL = 1
    ORDER_TIME_GTC = 0
...
class MockMT5:
    """Mock MetaTrader5 module for Linux/Docker environments"""
    
    # Constants
    ORDER_TYPE_BUY = MockMT5Constants.ORDER_TYPE_BUY
    ORDER_TYPE_SELL = MockMT5Constants.ORDER_TYPE_SELL
    TRADE_ACTION_DEAL = MockMT5Constants.TRADE_ACTION_DEAL
    ORDER_TIME_GTC = MockMT5Constants.ORDER_TIME_GTC
    ORDER_FILLING_IOC = MockMT5Constants.ORDER_FILLING_IOC
    TRADE_RETCODE_DONE = MockMT5Constants.TRADE_RETCODE_DONE
    
    @staticmethod
    def initialize():
        logger.info("Mock MT5: Initialize called")
        return True
    
    @staticmethod
    def login(login, password, server):
        logger.info(f"Mock MT5: Login called with login={login}, server={server}")
        return True
    ...
    def order_send(request):
        """Mock order send"""
        class OrderResult:
            retcode = MockMT5Constants.TRADE_RETCODE_DONE
            order = 12345
            price = request.get("price", 1.0501)
            comment = "Mock order executed"
        
        logger.info(f"Mock MT5: Order sent - {request}")
        return OrderResult()
...
# Use mock MT5 in Linux/Docker environment
try:
    import MetaTrader5 as mt5
    logger.info("Using real MetaTrader5 module")
except ImportError:
    mt5 = MockMT5()
    logger.info("Using mock MetaTrader5 module for Linux/Docker compatibility")
